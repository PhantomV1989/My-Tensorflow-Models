...
class NeuralNetwork():
    ...
    def train(self,trainset,Y_labels,epoch,output=True,save=True,load=False,printInterval=1000):          
        ...
        if(load &(os.path.exists(self.saverDir+self.saverName+'.meta'))):    
            ...
            ns.restore(sess, tf.train.latest_checkpoint('./'))#restore Variables
        else:
        ...
        for i in range(epoch):              
            ...
            if(i%printInterval==0):
                if (save):   
                    if(i==0):
                        saver.save(sess, self.saverDir+self.saverName,global_step=i,write_meta_graph=True)     
                    else:
                        saver.save(sess, self.saverDir+self.saverName,global_step=i,write_meta_graph=False)     
                if(output):                
...
if __name__=="__main__":    
    ...
    ann=NeuralNetwork(sizes=[100,50],lr=1)
    ann.train(x,y,5000,printInterval=1000,output=True,save=True,load=True)   
